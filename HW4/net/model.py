...
class WindowAttention(nn.Module):
    ...
    def forward(self, x):
        B_, N, C = x.shape  # B_ = num_windows * batch, N = window_size*window_size

        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads)
        q, k, v = qkv[:, :, 0], qkv[:, :, 1], qkv[:, :, 2]  # each shape: (B_, N, num_heads, head_dim)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Partition windows
        x_windows = window_partition(x, self.window_size)  # shape (nW*B, Ws*Ws, C)
        attn_windows = self.attn(x_windows)  # apply attention
...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        self.encoder_level1 = nn.Sequential(*[
            SwinTransformerBlock(dim=dim, input_resolution=(256, 256), num_heads=heads[0], window_size=8)
            for _ in range(num_blocks[0])
        ])

        self.down1_2 = Downsample(dim)

        self.reduce_noise_channel_2 = nn.Conv2d(int(dim * 2), int(dim * 2), kernel_size=1, bias=bias)
        self.encoder_level2 = nn.Sequential(*[
            SwinTransformerBlock(dim=int(dim * 2), input_resolution=(128, 128), num_heads=heads[1], window_size=8)
            for _ in range(num_blocks[1])
        ])

        self.down2_3 = Downsample(int(dim * 2))

        self.reduce_noise_channel_3 = nn.Conv2d(int(dim * 4) + 256, int(dim * 4), kernel_size=1, bias=bias)
        self.encoder_level3 = nn.Sequential(*[
            SwinTransformerBlock(dim=int(dim * 4), input_resolution=(64, 64), num_heads=heads[2], window_size=8)
