...
def tile_eval(model, input_, tile=128, tile_overlap=32):
    ...
    for h_idx in h_idx_list:
        for w_idx in w_idx_list:
            ...
            E[..., h_idx:(h_idx + tile),
              w_idx:(w_idx + tile)].add_(out_patch)
            W[..., h_idx:(h_idx + tile),
              w_idx:(w_idx + tile)].add_(out_patch_mask)
    restored = E.div_(W)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=int, default=3,
                        help='0 for denoise, 1 for derain, 2 for dehaze, '
                             '3 for all-in-one')

    parser.add_argument('--test_path', type=str, default="test/demo/",
                        help='save path of test images, can be directory or '
                             'an image')
    parser.add_argument('--output_path', type=str, default="output/demo/",
    ...
    parser.add_argument('--tile_size', type=int, default=128,
                        help='Tile size (e.g 720). None means testing on the '
    ...
    with torch.no_grad():
        for ([clean_name], degrad_patch) in tqdm(testloader):
            ...
            images_dict[clean_name[0] + '.png'] = (
                restored.cpu().numpy().squeeze()
            )
            save_image_tensor(restored,
                              opt.output_path + clean_name[0] + '.png')
